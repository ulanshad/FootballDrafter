<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="Ks.aspx.cs" Inherits="FootballDrafter.Ks" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div style="width: 790px; margin: 0px auto;">
        <asp:FormView ID="KFormView" runat="server" DataSourceID="KDetailDataSource" Width="260px"
        ...
        <asp:SqlDataSource ID="KDetailDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [playerName], [positionFull], [teamLong], [depth], [byeWeek] FROM [player_detail] WHERE ([playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="KsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:GridView ID="KGridVIew" runat="server" AutoGenerateColumns="False" CellPadding="4"
            DataSourceID="KDataSource" ForeColor="#333333" GridLines="None" 
            Width="530px">
            ...
            <Columns>
                ...
                <asp:BoundField DataField="pat" HeaderText="PAT" SortExpression="pat">
                    <ItemStyle HorizontalAlign="Right" />
                </asp:BoundField>
            </Columns>
        ...
        <asp:SqlDataSource ID="KDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [years].[year], [players_Ks].[points], [players_Ks].[ranking], [players_Ks].[fgm], [players_Ks].[pat] FROM [players_Ks] INNER JOIN [years] ON [players_Ks].[yearId] = [years].[yearId] WHERE ([players_Ks].[playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="KsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
    ...
    <div style="width: 790px; margin: 0px auto;">
        <asp:GridView ID="KsGridView" runat="server" AllowSorting="True" AutoGenerateColumns="False"
            CellPadding="4" DataSourceID="KsDataSource" ForeColor="#333333" GridLines="None"
            Width="790px" AllowPaging="True" PageSize="32" DataKeyNames="playerId" OnSorting="KsGridView_Sorting">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
        ...
        <asp:SqlDataSource ID="KsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT * FROM [players_Ks] WHERE (([playerName] LIKE '%' + @playerName + '%') AND ((@team = 'x' AND [team] <> 'FA') OR ([team] LIKE '%' + @team + '%')) AND ([rosterId] = @rosterId) AND ([yearId] = @yearId))">
            <SelectParameters>
