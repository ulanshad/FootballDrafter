<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="RBs.aspx.cs" Inherits="FootballDrafter.RBs" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div style="width: 990px; margin: 0px auto;">
        <asp:FormView ID="RBFormView" runat="server" DataSourceID="RBDetailDataSource" Width="260px"
        ...
        <asp:SqlDataSource ID="RBDetailDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [playerName], [positionFull], [teamLong], [depth], [byeWeek] FROM [player_detail] WHERE ([playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="RBsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:GridView ID="RBGridVIew" runat="server" AutoGenerateColumns="False" CellPadding="4"
            DataSourceID="RBDataSource" ForeColor="#333333" GridLines="None" 
            Width="730px">
        ...
        <asp:SqlDataSource ID="RBDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [years].[year], [players_RBs].[points], [players_RBs].[ranking], [players_RBs].[rushAttempts], [players_RBs].[rushYards], [players_RBs].[rushTDs], [players_RBs].[receptions], [players_RBs].[receivingYards], [players_RBs].[receivingTDs], [players_RBs].[fumbles] FROM [players_RBs] INNER JOIN [years] ON [players_RBs].[yearId] = [years].[yearId] WHERE ([players_RBs].[playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="RBsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
    ...
    <div style="width: 990px; margin: 0px auto;">
        <asp:GridView ID="RBsGridView" runat="server" AllowSorting="True" AutoGenerateColumns="False"
            CellPadding="4" DataSourceID="RBsDataSource" ForeColor="#333333" GridLines="None"
            Width="990px" AllowPaging="True" PageSize="60" DataKeyNames="playerId" OnSorting="RBsGridView_Sorting">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
        ...
        <asp:SqlDataSource ID="RBsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT * FROM [players_RBs] WHERE (([playerName] LIKE '%' + @playerName + '%') AND ((@team = 'x' AND [team] <> 'FA') OR ([team] LIKE '%' + @team + '%')) AND ([rosterId] = @rosterId) AND ([yearId] = @yearId))">
            <SelectParameters>
