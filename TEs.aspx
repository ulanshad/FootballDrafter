<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="TEs.aspx.cs" Inherits="FootballDrafter.TEs" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div style="width: 790px; margin: 0px auto;">
        <asp:FormView ID="TEFormView" runat="server" DataSourceID="TEDetailDataSource" Width="260px"
        ...
        <asp:SqlDataSource ID="TEDetailDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [playerName], [positionFull], [teamLong], [depth], [byeWeek] FROM [player_detail] WHERE ([playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="TEsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:GridView ID="TEGridVIew" runat="server" AutoGenerateColumns="False" CellPadding="4"
            DataSourceID="TEDataSource" ForeColor="#333333" GridLines="None" 
            Width="530px">
        ...
        <asp:SqlDataSource ID="TEDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT [years].[year], [players_TEs].[points], [players_TEs].[ranking], [players_TEs].[receptions], [players_TEs].[receivingYards], [players_TEs].[receivingTDs], [players_TEs].[fumbles] FROM [players_TEs] INNER JOIN [years] ON [players_TEs].[yearId] = [years].[yearId] WHERE ([players_TEs].[playerId] = @playerId)">
            <SelectParameters>
                <asp:ControlParameter ControlID="TEsGridView" Name="playerId" PropertyName="SelectedValue"
                    Type="Int32" />
            </SelectParameters>
    ...
    <div style="width: 790px; margin: 0px auto;">
        <asp:GridView ID="TEsGridView" runat="server" AllowSorting="True" AutoGenerateColumns="False"
            CellPadding="4" DataSourceID="TEsDataSource" ForeColor="#333333" GridLines="None"
            Width="790px" AllowPaging="True" PageSize="40" DataKeyNames="playerId" OnSorting="TEsGridView_Sorting">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
        ...
        <asp:SqlDataSource ID="TEsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:FantasyFootballConnectionString %>"
            SelectCommand="SELECT * FROM [players_TEs] WHERE (([playerName] LIKE '%' + @playerName + '%') AND ((@team = 'x' AND [team] <> 'FA') OR ([team] LIKE '%' + @team + '%')) AND ([rosterId] = @rosterId) AND ([yearId] = @yearId))">
            <SelectParameters>
